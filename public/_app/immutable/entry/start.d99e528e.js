import { o as Ce, t as ye } from '../chunks/index.2924c971.js';
import {
	S as Je,
	a as Ke,
	I as q,
	...
	P as Fe,
	h as We
} from '../chunks/singletons.c5232724.js';
function Xe(t, o) {
	return t === '/' || o === 'ignore'
		? t
		: o === 'never'
...
function Rt(t, o) {
	...
	let p = { branch: [], error: null, url: null },
		...
		H = !1,
		F,
		j = ($e = history.state) == null ? void 0 : $e[q];
	j || ((j = Date.now()), history.replaceState({ ...history.state, [q]: j }, '', location.href));
	const ue = z[j];
		...
		if ((y = v.universal) != null && y.load) {
			...
			const _ = {
				...
				async fetch(h, U) {
					...
					return (
						O(D.href),
						D.origin === r.origin && ($ = D.href.slice(r.origin.length)),
						x ? st($, D.href, U) : it($, U)
					);
				},
				setHeaders: () => {},
			...
			const A = p.branch[I],
				T =
					...
					((A == null ? void 0 : A.loader) !== E[1] ||
						Pe(O, L, R, (J = A.server) == null ? void 0 : J.uses, a));
			return T && (O = !0), T;
		});
		if (_.some(Boolean)) {
			try {
			...
			if (v[E])
				try {
					...
					if (h != null && h.includes(I)) (A = I.status ?? A), (T = I.error);
						...
						T = await X(I, { params: a, url: r, route: { id: s.id } });
					}
	...
	async function ce({
		...
		var $, D, E;
		const R = W(e, !1),
			L = xe({ url: e, type: c, delta: g, intent: R });
		if (!L) {
			y();
			return;
		}
		...
		(b = null),
			x ? ((p = _.state), _.props.page && (_.props.page.url = e), F.$set(_.props)) : Oe(_);
	...
	function Ye() {
		...
		function a(c, g) {
			const m = qe(c, f);
			if (!m) return;
			const { url: v, external: y, download: R } = we(m, K);
			if (y || R) return;
			const L = le(m);
			if (!L.reload)
				if (g <= L.preload_data) {
		...
		function s() {
			...
			for (const c of f.querySelectorAll('a')) {
				const { url: g, external: m, download: v } = we(c, K);
				if (m || v) continue;
				const y = le(c);
				y.reload ||
		...
		_start_router: () => {
			...
			(history.scrollRestoration = 'manual'),
				...
				f.addEventListener('click', (n) => {
					...
					if (!r) return;
					const { url: a, external: s, target: c, download: g } = we(r, K);
					if (!a) return;
					if (c === '_parent' || c === '_top') {
				...
				addEventListener('hashchange', () => {
					B && ((B = !1), history.replaceState({ ...history.state, [q]: ++j }, '', location.href));
				});
...
async function He(t, o) {
	...
	return new Promise(async (d) => {
		...
		let w = '';
		for (;;) {
			const { done: C, value: x } = await S.read();
			if (C && !w) break;
			for (
				w +=
					!x && w
